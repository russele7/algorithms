...
def find_right_break(encrypted_command: str, left_break_idx: int) -> int:
    ...
    for i in range(left_break_idx+1, len(encrypted_command)):
        ...
        if marker == 0:
            return i
        # Ритис, я тут не согласен.
        # Это условие НЕ относится к первым двум.
        # Его логичнее оставить внизу, т.к. оно никогда
        # не выполнитчя ПЕРЕД верхними двумя.
        # Я ради инетереса попробовал поднять его на верх
        # и тренажер не принял решение.
